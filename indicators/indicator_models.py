...
async def start_indicators(symbol: str, http_session: ClientSession):
    ...
    if not (initial_1m_data := await _get_initial_close_prices(symbol, http_session, '1m')):
        return
    if not (initial_4h_data := await _get_initial_close_prices(symbol, http_session, '4h')):
        return
