...
from bingx_command import ws_price, place_order, orders_book, account_balance, load_account_balances
...
async def buy_order_cmd(message: Message, session: AsyncSession, http_session: ClientSession):
    ...
    if (acc_money_usdt := await account_balance.get_balance('USDT')) < 1:
        return await message.answer(f'calc freezed volume is too small, volume: {acc_money_usdt}')
    ...
    for_commission = Decimal(str(for_commission)).quantize(Decimal(str(step_size)), rounding=ROUND_UP)
    execute_qty = Decimal(str(execute_qty)).quantize(Decimal(str(step_size)), rounding=ROUND_DOWN)
    execute_qty_c = float(execute_qty if acc_money > for_commission else execute_qty + max(for_commission, step_size))
    ...
    await message.answer(
        ...
        f'executed_qty_c: {execute_qty_c}\n'
    )
    ...
    execute_qty = float(execute_qty) if executed_qty_order == orig_qty_order else executed_qty_order

    data_for_db = {
        'price': price,
        'executed_qty': execute_qty,
        'cost': (cost := price * execute_qty),  # Цена за одну монету
        'commission': cost * TAKER_MAKER / 100,  # 1% комиссия(с запасом)
        'cost_with_commission': cost + (cost * TAKER_MAKER / 100),
    ...
    await gather(
        ...
        load_account_balances(http_session),  # Обновить балансы
    )
...
async def sell_order_cmd(message: Message, session: AsyncSession, http_session: ClientSession):
    ...
    if (last_order_data := await get_last_order(session, symbol)) is None:
    ...
    await gather(
        del_last_order(session, last_id),  # Удалить из базы
